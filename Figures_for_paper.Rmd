...
weights=read.csv('eezCHONE/rawdata.Canada-CHONe2014/weights/weights_All.csv')
#calculate weights, chose either "equal", "importance", "BWrank", "lmc", "lmc1", or "lmc10"
goals$weight <- reweigh(weights,"LG")
goals <- fix_FP_weights(goals,scores,rgn_id)

wts = with(subset(goals, !goal %in% goals_supra, c(goal, weight)), setNames(weight, goal))
rgn_name="COHI - Logit Coef + 0.1"
PlotFlower_small(main = rgn_name,
           lengths=x,
           widths=wts,
           fill.col=ifelse(is.na(x), 
...
weights=read.csv('eezCHONE/rawdata.Canada-CHONe2014/weights/weights_All.csv')
#calculate weights, chose either "equal", "importance", "BWrank", "lmc", "lmc1", or "lmc10"
goals$weight <- reweigh(weights,"LG1")
goals <- fix_FP_weights(goals,scores,rgn_id)
...
weights=read.csv('eezCHONE/rawdata.Canada-CHONe2014/weights/weights_All.csv')
#calculate weights, chose either "equal", "importance", "BWrank", "lmc", "lmc1", or "lmc10"
goals$weight <- reweigh(weights,"LG10")
goals <- fix_FP_weights(goals,scores,rgn_id)
...
**Figure 4**: Canadian modified OHI with LG1 weights from each province, numbers indicate weights

![](`r fig5`)

**Figure 5**: Canadian modified OHI with LG1 weights from each age group, numbers indicate weights
...
**This will be in the second paper**
![](`r fig6`)

**Figure 1**: relationship between LG1 weights and the median of each age group
